...
// Beginning of new function so the ULID is generated in the contract
export function internalGenerateULID() {
  let time = Date.now();
  const random = Math.random() * 0xfffffffffffff; // 6 bytes of randomness
  const id = new Uint8Array(16); // 16 bytes in total

  // Encode the time as 6 bytes
  id[0] = time & 0xff;
  id[1] = (time >> 8) & 0xff;
  id[2] = (time >> 16) & 0xff;
  id[3] = (time >> 24) & 0xff;
  ...
  // Encode the randomness as 6 bytes
  id[6] = random & 0xff;
  id[7] = (random >> 8) & 0xff;
  id[8] = (random >> 16) & 0xff;
  id[9] = (random >> 24) & 0xff;
  id[10] = (random >> 32) & 0xff;
  id[11] = (random >> 40) & 0xff;

  // Encode the ULID version (1) as 1 byte
  id[12] = 1;

  // Encode the reserved bits as 1 byte (currently always 0)
  id[13] = 0;

  // Calculate the ULID checksum
  let sum = 0;
  for (let i = 0; i < 14; i++) {
    sum = (sum * 0xff + id[i]) % 0xff;
  }
  id[14] = sum;
  ...
  let ulid = "";
  for (let i = 0; i < 15; i++) {
    ulid += "near.org-" + id[i].toString(36).padStart(2, "0");
  }
  return ulid;
}
...
export function internalResolveTransfer({
  ...
  let nftTransferLog = {
    ...
    // Version of the standard ("RODT-near.org-0.89.99").
