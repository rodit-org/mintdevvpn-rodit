...
// Collect the fee
export function collectDeposit(storageUsed: bigint) {
  ...
  let fee = attachedDeposit - requiredCost;
  near.log(`Collecting fee ${fee} yoctoNEAR`);

  //if the fee is greater than 1 yocto NEAR, we refund the predecessor that amount
  if (fee > 1) {
    // Send the money to the smart contract account
    const promise = near.promiseBatchCreate(near.currentAccountId());
    near.promiseBatchActionTransfer(promise, fee);
  }
}
